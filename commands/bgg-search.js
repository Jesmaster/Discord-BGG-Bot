...
module.exports = {
  ...
  bggSearch: async function (name) {
    const searchParams = new URLSearchParams(JSON.stringify({ query: name })),
    ...
    const response = await axios(
      "https://boardgamegeek.com/search/boardgame?q=" + encodeURI(name),
      {
        headers: {
  ...
  bggThing: async function (thing_id) {
    ...
    if (cache !== false) {
      console.log(
        ...
        `Found cached entry for ${thing_id}`,
      );
      return cache;
    }
  ...
  thingIdToSearchEmbed: async function (bggSearchResult, interaction) {
    if (bggSearchResult.found) {
      ...
      await interaction.reply(
        `No results found for "${interaction.options.getString("name")}".`,
      );
    }
  },
  /**
            ...
            try {
              ...
              collector
                ...
                .on("remove", (reaction, user) => {
                  ...
                  fields[field_delta].value = fields[field_delta].value.replace(
                    username,
                    "",
                  );
                ...
                .on("end", (collected) => {
                  ...
                  embed.setFooter({
                    text: "Reactions have been closed off for this suggestion.",
                  });
                  embedMessage.edit({ embeds: [embed] });
                });
              ...
              const deleteCollector = embedMessage.createReactionCollector({
                filter: deleteFilter,
              });
