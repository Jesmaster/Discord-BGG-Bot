...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("collection")
    .setDescription(
      "Search Boardgamegeek for collection info. Args: <username>",
    )
  ...
  bggCollection: async function (username) {
    const cache_type = "bgg_collection",
  ...
  collectionPrintEmbed: function (result, interaction, username) {
    ...
    if (hasErrors) {
      console.log(
        ...
        JSON.stringify(result.errors.error),
      );
    }
    ...
    if (isObject && !hasErrors && result.items["$"].totalitems > 0) {
      interaction.editReply({
        embeds: [
          this.collectionToEmbed(result, username, interaction?.member?.user),
        ],
      });
    } else {
  ...
  execute: async function (interaction) {
    ...
    while (result === "Building results" && attempts <= max_attemps) {
      console.log(
        new Date().toISOString(),
        `Building collection results for ${username} - Attempt ${attempts}`,
      );
    ...
    if (result === "Building results") {
      console.log(
        new Date().toISOString(),
        `Unable to build collection results after ${attempts} attemps for ${username}`,
      );
    } else {
      console.log(
        new Date().toISOString(),
        `Collection results found for ${username}`,
      );
    }
