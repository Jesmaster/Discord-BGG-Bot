...
mocha.describe("collectionToEmbed", function () {
  mocha.before(function (done) {
    fs.readFile(
      __dirname + "/data/bgg-collection-result.json",
      "utf8",
      function (err, fileContents) {
        ...
        embed = command
          .collectionToEmbed(bggCollectionResult, "jesmaster", user)
          .toJSON();
        done();
      },
    );
  ...
  mocha.it(
    "url should be https://boardgamegeek.com/collection/user/jesmaster",
    function () {
      assert.equal(
        embed.url,
        "https://boardgamegeek.com/collection/user/jesmaster",
      );
    },
  );
