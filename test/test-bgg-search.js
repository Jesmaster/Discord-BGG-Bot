...
mocha.describe("itemToSearchEmbed", function () {
  mocha.before(function (done) {
    fs.readFile(
      __dirname + "/data/bgg-thing-result.json",
      "utf8",
      function (err, fileContents) {
        ...
        embed = command
          .itemToSearchEmbed(bggThingResult.items.item[0], user)
          .toJSON();
        done();
      },
    );
  ...
  mocha.it(
    "url should be https://boardgamegeek.com/boardgame/177736",
    function () {
...
mocha.describe("thingIdFromBggSearch", function () {
  mocha.before(function (done) {
    fs.readFile(
      __dirname + "/data/bgg-search-result.json",
      "utf8",
      function (err, fileContents) {
        if (err) throw err;
  ...
  mocha.it("thing id should be 1653", function () {
    assert.equal(
      command.thingIdFromBggSearchCall(bggSearchResult).thing_id,
      "28720",
    );
  });
});
